...
// RunTagStore implements store.RunTagStore against SQLite.
type RunTagStore struct {
	db *sql.DB
}

func NewRunTagStore(db *sql.DB) *RunTagStore {
	return &RunTagStore{db: db}
}
...
//
// Postgres uses ANY($2::text[]); SQLite has no native array type, so we build
// a dynamic IN (?, ?, ...) clause and feed each run ID as a separate parameter.
...
func (s *RunTagStore) Add(ctx context.Context, projectID, runID, tag string) error {
	_, err := s.db.ExecContext(ctx, `
		INSERT INTO run_tags (id, project_id, run_id, tag)
		VALUES (?, ?, ?, ?)
		ON CONFLICT (project_id, run_id, tag) DO NOTHING
	`, uuid.NewString(), projectID, runID, tag)
	if err != nil {
...
func (s *RunTagStore) Delete(ctx context.Context, projectID, runID, tag string) error {
	_, err := s.db.ExecContext(ctx, `
		DELETE FROM run_tags WHERE project_id = ? AND run_id = ? AND tag = ?
	`, projectID, runID, tag)
	if err != nil {
