...
func (s *SpanStore) ListLLMSpansByRun(ctx context.Context, runID string) ([]*domain.Span, error) {
	rows, err := s.conn.Query(ctx, listLLMSpansByRunQuery, runID)
	if err != nil {
		return nil, fmt.Errorf("span_store list_llm_spans_by_run query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("span_store list_llm_spans_by_run scan: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("span_store list_llm_spans_by_run rows: %w", err)
	}
...
func (s *SpanStore) ListByProjectSince(ctx context.Context, projectID string, since time.Time) ([]*domain.Span, error) {
	rows, err := s.conn.Query(ctx, listByProjectSinceQuery, projectID, since)
	if err != nil {
		return nil, fmt.Errorf("span_store list_by_project_since query: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("span_store list_by_project_since scan: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("span_store list_by_project_since rows: %w", err)
	}

	return spans, nil
}

const countSinceQuery = `
SELECT count()
FROM spans
WHERE project_id = ?
...
func (s *SpanStore) CountSince(ctx context.Context, projectID string, window time.Duration) (int64, error) {
	row := s.conn.QueryRow(ctx, countSinceQuery, projectID, int64(window.Seconds()))
	var count uint64
